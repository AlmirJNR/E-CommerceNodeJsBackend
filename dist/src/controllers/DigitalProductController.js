...
router.get('/byName/:productName', async (request, response) => {
    response.json((await client.query('SELECT nome, imagem FROM mercadorias_digitais WHERE nome = $1', [request.params.productName.replace(/_/g, " ")])).rows);
});
...
router.get('/allProducts', async (request, response) => {
    ...
    response.json((await client.query('SELECT * FROM mercadorias_digitais LIMIT $1 OFFSET $2', [itemsPerPage, offset])).rows);
});
export default router;
//# sourceMappingURL=DigitalProductController.js.map