...
router.get('/byName/:productName', async (request, response) => {
    response.json((await client.query('SELECT nome, descricao, categoria, preco, quantidade, imagem FROM mercadorias_fisicas WHERE nome = $1', [request.params.productName.replace(/_/g, " ")])).rows);
});
...
router.get('/allProducts', async (request, response) => {
    ...
    response.json((await client.query('SELECT * FROM mercadorias_fisicas LIMIT $1 OFFSET $2', [itemsPerPage, offset])).rows);
});
export default router;
//# sourceMappingURL=PhysicalProductController.js.map